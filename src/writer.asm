...
/**
 * Effadce la ligne à l'adresse HL
 */
...
/**
 * Initialise la routine d'effacement
 */
initialize_effacement_ecran
    ; Reinitilise les adresses
    ld hl, (ADRESSE_ECRAN1) : ld (ADRESSE_ECRAN1_ACTUEL), hl : ld (gestion_caractere.adresse_ecran1+1), hl
    ld hl, (ADRESSE_ECRAN2) : ld (ADRESSE_ECRAN2_ACTUEL), hl : ld (gestion_caractere.adresse_ecran2+1), hl
...
.decalage_horizontal
    inc hl : inc hl : ld (.adresse_ecran1+1), hl
    inc bc : inc bc : ld (.adresse_ecran2+1), bc
 ...
 * HL, BC: les adresses ecran
 * DE :l 'adresse de la fonte
 */
affiche_caractere
    ld ixh, 9
.loop
...
/**
 * Attend avant d'effacer le writter
 */
writter_pause
    ld a, 50
/*
